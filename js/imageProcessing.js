...
const ImageProcessing = (() => {
  ...
  function sharpen(imageData) {
    ...
    // Read from a snapshot of the original pixels (so the convolution isn't
    // corrupted by already-sharpened neighbours), but write the result back
    // into the *original* imageData.data buffer in place. applyFilter() is
    // called for its side effect on the ImageData passed in by renderPage(),
    // which ignores the return value — so mutating in place (like every
    // other filter branch does) is required for the effect to actually show.
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        if (x === 0 || y === 0 || x === w - 1 || y === h - 1) {
          continue; // leave border pixels untouched (already correct in src)
        }
        for (let ch = 0; ch < 3; ch++) {
          ...
          for (let ky = -1; ky <= 1; ky++) {
            for (let kx = -1; kx <= 1; kx++) {
              ...
              sum += original[i] * kernel[k++];
            }
          }
          src[idx + ch] = clamp(sum);
        }
      }
    }
    return imageData;
