...
const EdgeDetector = (() => {
  ...
  const MAX_COMPONENTS_CONSIDERED = 6; // only the biggest few blobs can plausibly be the document
  ...
  function tryThreshold(mag, w, h, mean, std, kFactor) {
    ...
    // Only the largest few blobs can plausibly be the document outline —
    // running the hull/quad math on every small textured blob in a busy
    // scene is the main cost of this pipeline, so cap it here.
